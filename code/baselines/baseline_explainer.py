...
class PaGELink(nn.Module):
    ...
    def get_edge_mask(self, 
        ...
        with torch.no_grad():
            score = self.model(src_nid, tgt_nid, ghetero, feat_nids, feat_eids)
        ...
        for e in range(self.num_epochs):    
            ...
            score = self.model(src_nid, tgt_nid, ml_ghetero, feat_nids, None, ml_eweight_dict)
...
class HeteroGNNExplainer(nn.Module):
    ...
    def get_comp_g_edge_mask(self, src_nid, tgt_nid, ghetero, feat_nids, feat_eids):
        """
    ...
    def explain(self, src_nid, tgt_nid, ghetero, num_hops=2, trans=None):
        # Extract the computation graph (k-hop subgraph)
        (comp_g_src_nid, 
         comp_g_tgt_nid, 
         comp_g, 
        ...
        #     if comp_g.number_of_edges(etype=etype) == 0:
        #         i = 1
        #         break
...
class HeteroPGExplainer(nn.Module):
    ...
    def get_comp_g_edge_mask(self, src_nid, tgt_nid, ghetero, feat_nids, feat_eids, tmp = 1.0, training=False):
        """
        Get the explanation mask for the computation graph.
        ...
        with torch.no_grad():
            h = self.model.encode(ghetero, feat_nids, feat_eids)
    ...
    def explain(self, src_nid, tgt_nid, ghetero, trans=None, tmp = 1.0):
        # Extract the computation graph (k-hop subgraph) 
        (comp_g_src_nid, 
         comp_g_tgt_nid, 
         comp_g, 
