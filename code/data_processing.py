...
def load_dataset(dataset_dir, dataset_name, pred_etype, val_ratio, test_ratio, eval_exp=False):
    ...
    if 'synthetic' in dataset_name:
        src_ntype, tgt_ntype, pred_etype = 'user', 'item', 'likes'
    elif 'citation' in dataset_name:
        src_ntype, tgt_ntype, pred_etype = 'author', 'paper', 'likes'
...
'''
data processing for transE training
'''
def process_data4transE(g, 
                 val_ratio, 
                 test_ratio,
                 pred_etype = 'treats',
                 neg = 'pred_etype_neg'):
    ...
    graphs containing positive edges for train, valid, and test
    '''
    ...
    val_neg_dict = {}
    test_neg_dict = {}
    for cano_type in g.canonical_etypes:
    # if pred_etype not in cano_type:
        src_ntype, etype, tgt_ntype = cano_type
        ...
        train_pos_dict[cano_type] = (train_pos_u, train_pos_v)
        val_pos_dict[cano_type] = (val_pos_u, val_pos_v)
        test_pos_dict[cano_type] = (test_pos_u, test_pos_v)

        train_neg_dict[cano_type] = (train_neg_u, train_neg_v)
        val_neg_dict[cano_type] = (val_neg_u, val_neg_v)
        test_neg_dict[cano_type] = (test_neg_u, test_neg_v)
        
    # Construct graphs
    num_nodes_dict = get_num_nodes_dict(g)

    train_pos_g = dgl.heterograph(train_pos_dict, num_nodes_dict)
    val_pos_g = dgl.heterograph(val_pos_dict, num_nodes_dict)
    test_pos_g = dgl.heterograph(test_pos_dict, num_nodes_dict)

    train_neg_g = dgl.heterograph(train_neg_dict, num_nodes_dict)
    val_neg_g = dgl.heterograph(val_neg_dict, num_nodes_dict)
    test_neg_g = dgl.heterograph(test_neg_dict, num_nodes_dict)

    mp_g = remove_all_edges_of_etype(g, pred_etype)
    
    return mp_g, train_pos_g, train_neg_g, val_pos_g, val_neg_g, test_pos_g, test_neg_g


def load_data4transE(dataset_dir, dataset_name, pred_etype, val_ratio, test_ratio):
    '''
    Parameters
    ----------
    dataset_dir : string
    ...
    processed_g = process_data4transE(g, val_ratio, test_ratio, pred_etype)
    return g, processed_g